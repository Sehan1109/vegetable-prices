...
>
        ...
        <div class="bg-white border border-slate-100 rounded-3xl p-5 sm:p-8 shadow-xs">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
                ...
                <div class="lg:col-span-5 flex flex-col justify-between space-y-6">
                    ...
                    <!-- Callout Panel -->
                    <div class="bg-slate-900 text-white rounded-3xl p-6 border border-slate-800 shadow-md relative overflow-hidden flex flex-col justify-between min-h-[280px]">
                        <div class="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-emerald-500/10 to-transparent rounded-full -mr-10 -mt-10 pointer-events-none"></div>
                        
                        <div class="space-y-5 relative z-10">
                            <div class="flex justify-between items-center bg-slate-850 py-1.5 px-3 rounded-xl border border-slate-800 self-start">
                ...
                <div class="lg:col-span-7 flex flex-col items-center justify-center relative p-3 bg-slate-50 rounded-2.5xl border border-slate-100 min-h-[500px]">
                    ...
                    <div 
                        ...
                        class="absolute top-4 right-4 bg-slate-900 text-white border border-slate-800 rounded-xl p-3 shadow-lg z-20 w-52 text-[11px] pointer-events-none"
                    >
                        <template x-if="hoveredDistrict">
                            <div>
                                <div class="flex justify-between items-start border-b border-slate-800 pb-1 mb-1.5">
                                    <div>
                                        <h5 class="font-bold text-[11.5px]" x-text="hoveredDistrict.name"></h5>
                                        <span class="text-[8.5px] text-slate-450 font-mono uppercase font-semibold" x-text="hoveredDistrict.province"></span>
                                    </div>
                                    <span class="bg-slate-800 text-emerald-400 text-[8px] font-mono px-1 rounded">Spot</span>
                                </div>
                                <div class="space-y-1 font-mono">
                                    <div class="flex justify-between">
                                        <span class="text-slate-450 text-[10px]">Avg LKR Rate:</span>
                                        <strong class="text-white">Rs.<span x-text="hoveredDistrict.computedAveragePrice"></span>/kg</strong>
                                    </div>
                                    <div class="flex justify-between">
                                        <span class="text-slate-450 text-[10px]">Highest crop:</span>
                                        <span class="text-rose-400 text-[9.5px] truncate max-w-[85px]" x-text="hoveredDistrict.highestCropName"></span>
                                    </div>
                                    <div class="flex justify-between">
                                        <span class="text-slate-450 text-[10px]/none">Lowest crop:</span>
                    ...
                    <div class="absolute bottom-6 left-6 flex items-center gap-2 select-none opacity-45 pointer-events-none">
                        ...
                        <div class="font-mono text-[9px] text-slate-450 leading-none">
                            <span class="block font-bold">CEYLON PROJECTION</span>
                    ...
                    <div class="w-full max-w-[280px] xs:max-w-[325px] sm:max-w-[420px] aspect-[450/793] relative select-none">
                        <svg viewBox="0 0 450 793" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-full h-full drop-shadow-lg">
                            ...
                            <rect x="0" y="0" width="450" height="793" fill="url(#dot-mesh)" opacity="0.2" pointer-events="none" />

                            <!-- Simulated simplified geographic outline of Sri Lanka -->
                            <g filter="url(#subtle-shadow)">
                                <path d="M 120 10 Q 180 30 200 100 Q 250 150 280 250 Q 320 300 350 400 Q 370 500 340 600 Q 300 750 250 780 Q 200 790 100 750 Q 50 650 30 500 Q 10 350 50 200 Q 80 100 120 10 Z" fill="#cbd5e1" fill-opacity="0.2" stroke="#FFFFFF" stroke-width="2" />
                            </g>
                            ...
                            <template x-for="dist in calculatedMetrics" :key="dist.id">
                                ...
                                >
                                    ...
                                    <text :x="dist.cx" :y="dist.cy + 13" text-anchor="middle" 
                                          :class="{'fill-slate-950 font-black text-[8.5px]': selectedDistrictId === dist.id, 'fill-slate-800 text-[8px]': hoveredDistrictId === dist.id, 'fill-slate-650 font-bold opacity-80 text-[7.2px]': selectedDistrictId !== dist.id && hoveredDistrictId !== dist.id}"
    ...
    <div x-show="showGlassModal" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div 
            ...
            class="relative w-full max-w-xl backdrop-blur-xl bg-slate-950/80 border border-white/10 rounded-3xl p-6 sm:p-8 shadow-[0_20px_50px_rgba(0,0,0,0.4)] overflow-hidden z-10 text-white"
        >
            <!-- Glowing Orbs -->
            <div class="absolute -top-1/4 -right-1/4 w-72 h-72 bg-emerald-500/15 rounded-full blur-3xl pointer-events-none"></div>
            <div class="absolute -bottom-1/4 -left-1/4 w-72 h-72 bg-blue-500/10 rounded-full blur-3xl pointer-events-none"></div>

            <button @click="showGlassModal = false" type="button" class="absolute top-4 right-4 p-2 bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/25 rounded-full transition-all cursor-pointer flex items-center justify-center text-slate-300 hover:text-white">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6 relative z-10">
                <div class="bg-white/5 border border-white/10 rounded-2xl p-4 backdrop-blur-md relative overflow-hidden flex flex-col justify-between">
                    <div>
                        ...
                        <strong class="text-3xl font-mono font-black text-white block mt-3">
                            Rs. <span x-text="selectedDistrict.computedAveragePrice"></span> <span class="text-xs text-slate-400 font-normal">/kg</span>
                        </strong>
                    </div>
                    <p class="text-[10px] text-slate-450 mt-2">Calculated from active central market trading nodes.</p>
                </div>
                ...
                <div class="bg-white/5 border border-white/10 rounded-2xl p-4 backdrop-blur-md relative overflow-hidden flex flex-col justify-between">
                    <div class="flex justify-between items-start">
                        <div>
                            ...
                            <strong :class="selectedDistrict.trendPercent <= 0 ? 'text-emerald-400' : 'text-rose-450'" class="text-2xl font-mono font-black block mt-3">
                                <span x-text="selectedDistrict.trendPercent <= 0 ? '-' : '+'"></span><span x-text="Math.abs(selectedDistrict.trendPercent) + '%'"></span>
                            </strong>
                        </div>
                        <div :class="selectedDistrict.trendPercent <= 0 ? 'text-emerald-400' : 'text-rose-400'" class="p-2 rounded-xl bg-white/5 border border-white/10">
                            <i :data-lucide="selectedDistrict.trendPercent <= 0 ? 'trending-down' : 'trending-up'" class="w-5 h-5"></i>
                        </div>
                    </div>
                    <p class="text-[10px] text-slate-450 mt-2">Delta relative to previous week's baseline (Rs. <span x-text="selectedDistrict.computedAveragePricePrevWeek"></span>).</p>
                </div>
            </div>
            ...
            <div class="space-y-3 mb-6 relative z-10">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                    <div class="bg-white/5 border border-emerald-500/20 rounded-xl p-3.5 backdrop-blur-sm">
                        ...
                        <h5 class="font-bold text-[13px] text-white mt-1.5 truncate" x-text="selectedDistrict.lowestCropName"></h5>
                        <strong class="text-emerald-300 font-mono text-[12px] mt-0.5 block">Rs. <span x-text="selectedDistrict.lowestCropPrice"></span> /kg</strong>
                    </div>

                    <div class="bg-white/5 border border-rose-500/20 rounded-xl p-3.5 backdrop-blur-sm">
                        ...
                        <h5 class="font-bold text-[13px] text-white mt-1.5 truncate" x-text="selectedDistrict.highestCropName"></h5>
                        <strong class="text-rose-300 font-mono text-[12px] mt-0.5 block">Rs. <span x-text="selectedDistrict.highestCropPrice"></span> /kg</strong>
                    </div>
                </div>
            </div>

            <div class="bg-white/5 border border-white/10 rounded-2xl p-4.5 backdrop-blur-md relative z-10">
                ...
                <div class="flex flex-wrap gap-2">
                    <template x-for="crop in selectedDistrict.primaryCrops" :key="crop">
                        <span class="px-2.5 py-1 text-xs font-semibold bg-white/5 border border-white/10 text-emerald-200 rounded-lg transition-all hover:bg-white/10" x-text="crop"></span>
                    </template>
                </div>
            </div>
